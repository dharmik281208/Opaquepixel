...
def _capacity_bits(height: int, width: int) -> int:
    blocks_h = height // BLOCK
    blocks_w = width // BLOCK
    return blocks_h * blocks_w * BITS_PER_BLOCK
...
def get_image_capacity_bytes(height: int, width: int) -> int:
    return _capacity_bits(height, width) // 8
...
def hide_payload_in_image(
    ...
    inside an image carrier using DST/DCT frequency-domain steganography.
    """
    from engines.payload import bytes_to_bits, package_payload, prepend_length_header
    ...
    h, w = cover.shape[:2]
    capacity_bytes = get_image_capacity_bytes(h, w)
    if len(payload_bits) > capacity_bytes * 8:
        raise ValueError("Payload exceeds DST carrier capacity")
...
def reveal_payload_from_image(
    ...
    """
    High-level handler to extract ANY hidden payload from a DST/DCT image stego carrier.
    """
    from engines.payload import bits_to_bytes, extract_length_header, unpackage_payload
    ...
    h, w = stego.shape[:2]
    max_capacity_bits = get_image_capacity_bytes(h, w) * 8

    # Extract 32-bit length header via DST blocks
    ...
    if length < 0 or length > max_payload_bits:
        raise ValueError("Invalid or corrupted stego image (payload length exceeds DST capacity)")
